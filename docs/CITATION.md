...
- **Reader** (`ui/ReaderScreen`, `ReaderViewModel`): renders the internal model — format-blind — as a
  ...
  - **Reading comfort:** a Display sheet covering type (size, spacing, margins, tracking, face —
    ...
    justification, hyphenation, paragraphs indented, spaced or **both**), colour (Paper / Sepia /
    Night / System as the starting point, with the **page, text, headings and links each following
    that theme or set to a colour of your own, independently**, all of them on and off together in
    one switch, true black, warmth, in-reader brightness) and screen behaviour (keep awake, full screen, orientation, volume keys) — persisted,
...
Typography follows book convention rather than web convention by default: paragraphs separated by a
first-line indent rather than a blank line, and no mark at all on the paragraph opening a section —
though the two marks are settings, and can be used together. Royal Road
...
## Reading mechanics — search, bookmarks, progress, lookup, references (core built + verified)
...
| **Notes and links in the text** | `reader/BookReferences`, `ui/reader/RenderedReference` | A footnote marker was drawn as a superscript in the link colour and did nothing when tapped — a control that lies about itself, in a book whose producer had already said exactly what it pointed at. The structure layer had done the work (`InlineSpan.href` resolved to zip-absolute, `FOOTNOTE_REF` told apart from an ordinary link, `Chapter.anchors` mapping every `id` to an offset); nothing read it. Now a **note reference opens the note over the page** — a footnote is read *in* a sentence, not instead of it, which is the whole reason the producer marked it as a note rather than a link — an **internal link jumps**, and an **external one asks first**, because tapping a link in a book hands your book to a browser. A long endnote is cut on a word boundary with the way to the rest offered, rather than the sheet growing into a chapter. A reference naming a fragment the book does not contain resolves to **nothing at all**: landing at the top of the notes file and showing note 1 in answer to a tap on note 17 is not a degraded answer, it is a confident wrong one, and the reader is told instead. |
| **Coming back from a jump** | `reader/ReaderHistory`, `ReadingPlace` | The thing that makes a jump safe to take. Following a note, a cross-reference, a search hit or a contents entry used to cost you the paragraph you were in — you came back by scrolling for it, which is enough of a cost that people stop following references at all. A bounded stack of places (jumps nest: a note that cites another note), with the system back button and a bar over the page both undoing one, and a dismiss for the jump that was not a peek. Deliberately **not** persisted: a jump you took yesterday is not one you are in the middle of, and offering to undo it on open would be offering to leave the page you asked for. Fixing it also fixed the channel those jumps travelled on — a canonical character offset was being staged on the one the *scrolling* reader reads as a pixel count, so a contents entry or a search hit pointing inside a chapter landed correctly in paged mode and some arbitrary distance down the page in the other. |
...
| Area | What it does |
|---|---|
| **Typography** | Size, line spacing, margins, letter spacing, and how a paragraph is marked off from the one above: **indented**, **spaced**, or **both**. The first two are correct and belong to different traditions; the third belongs to neither and is the one readers keep asking for, because an indent alone marks where a paragraph begins without giving the eye anywhere to rest — at a tight line spacing that reads as one unbroken column — and a blank line alone loses the mark, which a line of dialogue ending flush right makes genuinely ambiguous. The paragraph opening a section carries neither mark, whichever is set: that is the convention every printed book follows and the reason an indent does not look like a mistake. The setting reaches the Kindle and O'Reilly readers too, because a reader who has decided how their paragraphs are separated decided it about *reading*, not about EPUBs. Justification and automatic hyphenation are paired, and the sheet says so: justification on a narrow phone column *without* hyphenation is what opens rivers of whitespace. Justification applies to running prose only — a justified heading or table row stretches a few words across the column and reads as a bug; verse and code are never justified, because their line breaks are the author's. |
...
| **Your own colours** | Not a theme but four **roles** — page, text, headings, links (`ReaderColorRole`) — each independently either following the theme or set to a colour you name, by tapping a swatch or typing an exact `#RRGGBB`. Both ways in, because they answer different questions: a strip of tints is for *comparing* which is easiest on your eyes; a hex field is for a reader who arrives already knowing the value, from an overlay they own or a colour they were given, and for whom hunting it on a gradient would be guesswork. The presets are the usual answers but not everybody's: readers with Irlen syndrome or a light sensitivity are routinely told a specific tint helps, and so are many dyslexic readers, and no preset could guess it. Splitting the roles is what makes the *partial* answers reachable — keep the page following the system into dark mode and set only the prose; keep Sepia's page and give headings a second colour — where a single `CUSTOM` theme made you give up following the system to change one colour, and then name everything by hand. A role handed back to the theme **keeps its colour** for the next time it is taken, since "Auto" is a button readers press to compare. One switch — `useCustomColors` — puts the whole palette on the page or takes it all off, because alternating between the app's colours and your own is how anybody decides they like them and has to stay one tap in each direction; which roles are yours and what colour each one is both survive being switched off, so it is an A/B rather than a decision. Switching it on for the first time takes over the page and the text in the colours already on screen, which is what the old `CUSTOM` chip did and the reason it read as an adjustment to the page you nearly liked rather than a blank one. The result is shown as a **sample page** — the sheet covers the book, so otherwise you are picking colours against a preview you cannot see — warmed exactly as the real page will be, and under the System theme it says so, because a colour chosen against a light page is the colour that vanishes at sunset. Colours are forced opaque (a part-transparent page would let the app's own surface bleed through the book). Contrast is **checked, not policed**: a pair below WCAG AA is called out as hard to read, but the reader is the one looking at the page and may have a reason for it. Rows written before the split are read back by `ReaderColorMigration`: the old `CUSTOM` theme becomes the page and the text taken over, switched on, in the colours it held, and every other theme keeps its page with its held colours still merely held. |
...
reading, footnotes that open over the page and links that go where they say, with a way back from
every jump, a screen that stays on, and word lookup from the selection — and it is set the way you set

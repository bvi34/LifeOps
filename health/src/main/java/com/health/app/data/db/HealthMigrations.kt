...
/**
 * Every schema step `health.db` has taken, in one place.
 *
 * They live beside [HealthDatabase] rather than inside its companion because they are the file that
 * only ever grows: nine migrations is already more lines than the database declaration, the table
 * list and the singleton put together, and a reader opening `HealthDatabase.kt` is almost never
 * looking for what v4 did to the medicine cabinet.
 *
 * Each one carries **why**, not just what. A migration is the one piece of code nobody can re-derive
 * from the current schema — the reasoning about what was not backfilled, and what was deliberately
 * left untouched, is only recoverable if it was written down at the time.
 */
object HealthMigrations {

...
enum class BlockType(val key: String, val label: String, val marker: String) {
    ...
    val isProse: Boolean get() = this != CODE && this != DIVIDER
 ...
 *
 * Everything here is pure and line-oriented; there is deliberately no inline parsing. Bold and
 * italic stay as the asterisks you typed, because half-parsing Markdown — structure yes, emphasis
 * no — is the version that loses least, and a block editor that silently eats a literal `*` in a
 * shell command is worse than one that shows it.
 */
object DocBlocks {
    ...
    fun render(blocks: List<DocBlock>): String {
        ...
        blocks.forEach { block ->
            if (block.type == BlockType.NUMBERED) ordinal++ else ordinal = 0
    ...
    fun wordCount(blocks: List<DocBlock>): Int =
        blocks.filter { it.type.isProse }.sumOf { wordCount(it.text) }
    ...
    fun preview(blocks: List<DocBlock>, maxChars: Int = 120): String {
        val text = blocks.firstOrNull { it.type == BlockType.PARAGRAPH && it.text.isNotBlank() }?.text

...
object SuiteGlyphs {
    ...
    val Thermometer: ImageVector by lazy {
        glyph("SuiteThermometer") {
            ...
            line(width = 1.8f) { moveTo(12f, 10.6f); lineTo(12f, 14.6f) }
            solid { circle(cx = 12f, cy = 16.6f, r = 2.2f) }
            // Scale ticks — texture at tile size, gone at settings size, missed at neither.
            line(width = 1.4f, alpha = 0.5f) {
     ...
     * under an arch.
     */
    val Household: ImageVector by lazy {
    ...
    /** Project — a board of work in columns: outlines, docs, lore. */
    val Board: ImageVector by lazy {
        glyph("SuiteBoard") {
            ...
            line(width = 1.3f, alpha = 0.45f) { moveTo(3.2f, 7.9f); lineTo(20.8f, 7.9f) }
    ...
    /** Logistics — a basket: pantry, groceries, recipes. */
    ...
    /** Citation — an open book with text on both pages. */
    ...
    /** Advisor — an answer coming back: a bubble with a spark in it. */
    val AnswerSpark: ImageVector by lazy {
